...
class TestGuiV2VisualSystem(unittest.TestCase):
    ...
    def test_schema_v2_header_uses_secondary_button_tokens(self) -> None:
        ...
        root_content = getattr(screen, "_root_content", None)
        self.assertIsNotNone(root_content)
        if root_content is None:
            self.fail("Schema v2 screen must expose _root_content frame for token assertions.")
        frame_children = [child for child in root_content.winfo_children() if isinstance(child, tk.Frame)]
